...
def create_sub_image_grid(image, mask, grid_size=100):
    ...
    for i in range(0, H - gh, gh):
        for j in range(0, W - gw, gw):
            sub_image = image[i:i + gh, j:j + gw, :].reshape(1, 100, 100, bands)
            sub_mask = mask[i:i + gh, j:j + gw].reshape(1, 100, 100)
...
def main():
    ...
    for image, mask in zip(images, masks):
        sub_x, sub_y = create_sub_image_grid(image, mask)
    ...
    N, *_ = weights.shape
    selection = np.random.choice(N, size=100, replace=False, p=weights)
    X = X[selection]
    Y = Y[selection]
